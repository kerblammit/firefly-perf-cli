...
if [ $# -ne 9 ]; then
    ...
    printf "${RED}6. Existing contract address\n"
    printf "${RED}7. Existing token pool name\n"
    printf "${RED}8. Existing token pool block number\n"
    printf "${RED}9. Transaction signing key\n"
    printf "${RED}ex: ./prepForRemote.sh https://my.firefly.node/ my/prefix myuser mypassw0rd mynamespace 0xe7ca06eabfb44ef4d8de55fe2b4e0e42661b7209 mytoken 45659 0xa049c3dc2a7015a6188b2dd061fd519136c725bc${NC}\n"
    exit 1
fi
...
FIREFLY_NAMESPACE=$5
EXISTING_CONTRACT_ADDR=$6
EXISTING_TOKEN_POOL_NAME=$7
EXISTING_TOKEN_POOL_BLOCK=$8
SIGNING_KEY=$9
...
instances:
  - name: long-run
    ...
    tokenOptions:
      ...
      config:
        blockNumber: ${EXISTING_TOKEN_POOL_BLOCK}
        address: ${EXISTING_CONTRACT_ADDR}
EOF
